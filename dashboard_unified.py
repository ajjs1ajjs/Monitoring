...
def get_server_metrics(server_id: int, range: str = "1h"):
    ...
    try:
        ...
        # Parse data
        labels = [
            row[0].split("T")[1].split(".")[0].split(":")[0] + ":" + row[0].split("T")[1].split(".")[0].split(":")[1]
            for row in rows
        ]
...
def get_dashboard_metrics():
    ...
    try:
        ...
        return {
            ...
            "summary": {"avg_cpu": avg_cpu, "avg_memory": avg_memory, "avg_disk": avg_disk},
        }
    finally:
        conn.close()
    ...
    try:
        ...
        for row in rows:
            ...
            if server_id not in data_by_server:
                data_by_server[server_id] = {"cpu": [], "memory": [], "disk": [], "network_rx": [], "network_tx": []}
        ...
        # Calculate averages
        avg_cpu = (
            sum(sum(v["cpu"]) for v in data_by_server.values()) / sum(len(v["cpu"]) for v in data_by_server.values())
            if data_by_server
            else 0
        )
        avg_memory = (
            sum(sum(v["memory"]) for v in data_by_server.values())
            / sum(len(v["memory"]) for v in data_by_server.values())
            if data_by_server
            else 0
        )
        avg_disk = (
            sum(sum(v["disk"]) for v in data_by_server.values()) / sum(len(v["disk"]) for v in data_by_server.values())
            if data_by_server
            else 0
        )
...
def get_dashboard_status():
    ...
    try:
        ...
        return {
            "servers": {"online": online, "offline": offline, "total": online + offline},
            "disk": {"avg": disk_stats[0] if disk_stats[0] else 0, "max": disk_stats[1] if disk_stats[1] else 0},
            "network": {
    ...
    try:
        ...
        for row in rows:
            ...
            if ts not in seen_timestamps:
                ...
                net_val = ((row["network_rx"] or 0) + (row["network_tx"] or 0)) / 1024 / 1024
