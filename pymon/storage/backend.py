    ...
    async def read(
        ...
        async with self._lock:
            for key, points in self._data.items():
                ...
                if labels:
                    key_labels = key[len(key_prefix):]
                    expected = ",".join(f"{k}={v}" for k, v in sorted(labels.items()))
                    if key_labels != expected:
                        continue
                for point in points:
...
class SQLiteStorage(StorageBackend):
    ...
    async def read(
        ...
        result = []
        async with aiosqlite.connect(self._db_path) as db:
            query = "SELECT timestamp, value FROM metrics WHERE name = ? AND timestamp BETWEEN ? AND ?"
            params: list = [name, start.isoformat(), end.isoformat()]
            if labels:
                query += " AND labels = ?"
