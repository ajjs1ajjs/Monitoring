...
class ScrapeManager:
    ...
    async def _evaluate_rules(self):
        ...
        try:
            cursor = await conn.execute("SELECT id, name, cpu_percent, memory_percent, disk_percent, last_status FROM servers WHERE enabled=1")
            servers = await cursor.fetchall()
            if not hasattr(self, '_rule_state'):
                self._rule_state = {}
            import time as _time
            now_ts = _time.time()
            for srv in servers:
                sid, sname, cpu, mem, disk, _ = srv
                vals = {"cpu": cpu or 0, "memory": mem or 0, "mem": mem or 0, "disk": disk or 0}
                for cfg_rule in self.config.alerting_rules:
                    metric_name = next((kw for kw in ("cpu", "memory", "mem", "disk") if kw in cfg_rule.expr.lower()), None)
                    if metric_name is None:
                        continue
                    val = vals.get(metric_name, 0)
                    key = (cfg_rule.name, sid)
                    state = self._rule_state.get(key, {"phase": "ok", "since": 0, "notified_firing": False})
                    if val > cfg_rule.threshold:
                        if state["phase"] == "ok":
                            state["phase"] = "pending"
                            state["since"] = now_ts
                        elif state["phase"] == "pending" and (now_ts - state["since"]) >= cfg_rule.duration:
                            state["phase"] = "firing"
                            if not state.get("notified_firing"):
                                state["notified_firing"] = True
                                await self._trigger_alert(
                                    f"⚠️ {cfg_rule.name} on {sname}",
                                    f"Rule '{cfg_rule.name}' triggered: {cfg_rule.expr} = {val:.1f} (threshold: {cfg_rule.threshold}). {cfg_rule.message}"
                                )
                    else:
                        if state["phase"] in ("pending", "firing"):
                            state["phase"] = "ok"
                            state["notified_firing"] = False
                            if state.get("notified_firing"):
                                await self._trigger_alert(
                                    f"✅ Resolved: {cfg_rule.name} on {sname}",
                                    f"Rule '{cfg_rule.name}' returned to normal: {cfg_rule.expr} = {val:.1f}"
                                )
    ...
    async def _trigger_alert(self, title, message):
        try:
            ...
            if data.get("slack_webhook_url"):
                channels["slack"] = {"webhook_url": data["slack_webhook_url"]}
...
async def _run_backup_if_due(config):
    ...
    try:
        ...
        dt_now = datetime.now()
        if hour != '*' and int(hour) != dt_now.hour:
            return
        if minute != '*' and int(minute) != dt_now.minute:
            return
