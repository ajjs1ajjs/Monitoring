...
function updateLiveTable(data) {
    ...
    const html = targetData.map(n => `
        <tr style="cursor: pointer;" onclick="openDrawer(${n.id})">
            <td>
                    <div class="status-badge ${n.last_status === 'up' ? 'up' : 'down'}" title="${escapeHtml(n.error_message || '')}">
                    <span class="status-dot ${n.last_status === 'up' ? 'pulse' : ''}"></span>
                    ${escapeHtml(n.last_status || 'unknown')}
                </div>
            </td>
            <td>
                ...
                <div style="padding: 0.85rem 1rem; border-bottom: 1px solid var(--border); display: flex; align-items: center; gap: 1rem; transition: background 0.2s;">
                    ...
                    <div style="flex: 1;">
                        <div style="display: flex; justify-content: space-between;">
                            <span style="font-weight: 600; color: #fff;">${escapeHtml(l.action)}</span>
                            <span style="font-size: 0.7rem; color: var(--text-muted);">${l.timestamp ? escapeHtml(l.timestamp.slice(0, 19).replace('T', ' ')) : '-'}</span>
                        </div>
                        <div style="font-size: 0.75rem; color: var(--text-muted); margin-top: 0.1rem;">
                            Target: <span style="color: #94a3b8;">${escapeHtml(l.target || 'System')}</span> • Details: <span style="color: #94a3b8;">${escapeHtml(l.details || 'No additional info')}</span>
                        </div>
                    </div>
                </div>
        ...
        feed.innerHTML = logs.map(log => `
            <div class="alert-item-v2 ${log.action.toLowerCase().includes('critical') ? 'critical' : log.action.toLowerCase().includes('warning') ? 'warning' : ''}">
                <div class="alert-header">
                    <span class="alert-title">${escapeHtml(log.action)}</span>
...
setInterval(populateServerSelect, 120000);
    ...
    tbody.innerHTML = services.map(s => `
        <tr>
            <td>
                ...
                <div style="font-size: 0.7rem; color: var(--text-muted);">${escapeHtml(s.target_url)}</div>
            </td>
