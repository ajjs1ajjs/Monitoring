...
def main():
    ...
    if args.command == "server":
        try:
            ...
            # Keep the strong random password generated at import time when the
            # config value is empty or a known weak default — never fall back to
            # an empty admin password.
            cfg_pw = config.auth.admin_password
            if cfg_pw and cfg_pw not in ("change-me-on-first-login", "291263"):
                auth_cfg.admin_password = cfg_pw
        ...
        try:
            ...
            from pymon.auth import hash_password

            # Allow an explicit password via env, otherwise generate a strong random one.
