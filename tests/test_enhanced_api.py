...
class TestMetricsHistory:
    ...
    def test_get_metrics_history_all_metrics(self, db):
        """Test getting all metrics"""
        from pymon.web_dashboard_enhanced import get_servers_metrics_history

        result = get_servers_metrics_history(range="1h")
    ...
    def test_get_metrics_history_cpu_only(self, db):
        """Test getting only CPU metric"""
        from pymon.web_dashboard_enhanced import get_servers_metrics_history

        result = get_servers_metrics_history(range="1h", metric="cpu")
    ...
    def test_get_metrics_history_memory_only(self, db):
        """Test getting only Memory metric"""
        from pymon.web_dashboard_enhanced import get_servers_metrics_history

        result = get_servers_metrics_history(range="1h", metric="memory")

        assert "labels" in result
        assert len(result["datasets"]) == 1
        assert result["datasets"][0]["label"] == "Memory"

    def test_get_metrics_history_single_server(self, db):
        """Test getting metrics for specific server"""
        from pymon.web_dashboard_enhanced import get_servers_metrics_history

        result = get_servers_metrics_history(server_id=1, range="1h")

        assert "labels" in result
        assert "datasets" in result

    def test_get_metrics_history_invalid_range(self, db):
        """Test with invalid time range"""
        from pymon.web_dashboard_enhanced import get_servers_metrics_history

        # Should default to 1h or handle gracefully
        result = get_servers_metrics_history(range="invalid")
...
class TestDiskBreakdown:
    ...
    def test_get_disk_breakdown_multiple_volumes(self, db):
    ...
    def test_get_disk_breakdown_nonexistent_server(self, db):
        """Test disk breakdown for non-existent server"""
        from pymon.web_dashboard_enhanced import get_server_disk_breakdown

        result = get_server_disk_breakdown(999)

        assert "disks" in result
        assert len(result["disks"]) == 0

    def test_get_disk_breakdown_no_disk_info(self, db):
        ...
        result = get_server_disk_breakdown(2)
...
class TestExport:
    ...
    def test_export_json(self, db):
        ...
        result = export_server_data(1, format="json", range="24h")
    ...
    def test_export_csv(self, db):
        ...
        result = export_server_data(1, format="csv", range="24h")
    ...
    def test_export_nonexistent_server(self, db):
        """Test export for non-existent server"""
        from pymon.web_dashboard_enhanced import export_server_data

        result = export_server_data(999, format="json", range="24h")
...
class TestCompare:
    ...
    def test_compare_cpu(self, db):
        """Test CPU comparison"""
        from pymon.web_dashboard_enhanced import compare_time_ranges

        result = compare_time_ranges(metric="cpu", range="1h")
    ...
    def test_compare_memory(self, db):
        """Test Memory comparison"""
        from pymon.web_dashboard_enhanced import compare_time_ranges

        result = compare_time_ranges(metric="memory", range="1h")

        assert "current" in result
        assert "trend" in result

    def test_compare_disk(self, db):
        """Test Disk comparison"""
        from pymon.web_dashboard_enhanced import compare_time_ranges

        result = compare_time_ranges(metric="disk", range="1h")

        assert "current" in result
        assert "trend" in result

    def test_compare_network(self, db):
        """Test Network comparison"""
        from pymon.web_dashboard_enhanced import compare_time_ranges

        result = compare_time_ranges(metric="network", range="1h")

        assert "current" in result
        assert "trend" in result

    def test_compare_single_server(self, db):
        """Test comparison for specific server"""
        from pymon.web_dashboard_enhanced import compare_time_ranges

        result = compare_time_ranges(server_id=1, metric="cpu", range="1h")
...
class TestDataValidation:
    ...
    def test_metrics_history_data_types(self, db):
    ...
    def test_disk_breakdown_data_types(self, db):
    ...
    def test_uptime_percent_range(self, db):
        """Test that uptime percent is in valid range"""
        from pymon.web_dashboard_enhanced import get_server_uptime_timeline

        result = get_server_uptime_timeline(1, days=7)

        uptime = result["uptime_percent"]
        assert 0 <= uptime <= 100

    def test_compare_trend_values(self, db):
